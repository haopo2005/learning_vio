    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)  //measurement.second: img points, measurement.first: IMU, 角速度及角速度
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                if (t <= img_t)//如果IMU时间戳t在相邻两帧的时间戳之间，将当前时间设置为t,直接读取角速度和线加速度
                {
                    if (current_time < 0)
                        current_time = t;
                    double dt = t - current_time;//JST：多次IMU观测之间的时间间隔
                    assert(dt >= 0);
                ...
                else//如果imu的时间戳t大于第k+1帧的图像时间戳img_t, current_time < img_time < t 
                {
                    double dt_1 = img_t - current_time;//第k+1帧图片与上一次IMU数据的时间差
                    double dt_2 = t - img_t;//当前IMU观测时间与第k+1帧图的时间差
                    current_time = img_t;// 上一个IMU数据的时间戳
                    assert(dt_1 >= 0);
                    assert(dt_2 >= 0);
                    assert(dt_1 + dt_2 > 0);
                    double w1 = dt_2 / (dt_1 + dt_2);
                    double w2 = dt_1 / (dt_1 + dt_2);
                    dx = w1 * dx + w2 * imu_msg->linear_acceleration.x();//基于时间加权各个IMU测量值
