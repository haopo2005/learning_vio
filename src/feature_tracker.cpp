...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0)
    {
        ...
        cur_img ：buildOpticalFlowPyramid构造的第一个8位输入图像或金字塔。
        forw_img ：与prevImg相同大小和相同类型的第二个输入图像或金字塔
        cur_pts ：需要找到流的2D点的矢量(vector of 2D points for which the flow needs to be found;);点坐标必须是单精度浮点数。
        forw_pts ：输出二维点的矢量（具有单精度浮点坐标），包含第二图像中输入特征的计算新位置;当传递OPTFLOW_USE_INITIAL_FLOW标志时，向量必须与输入中的大小相同。
...
void FeatureTracker::undistortedPoints()
{
    ...
    for (unsigned int i = 0; i < cur_pts.size(); i++)
    {
        ...
        Eigen::Vector3d b;
        m_camera->liftProjective(a, b);//见EquidistantCamera::liftProjective, Lifts a point from the image plane to its projective ray --> b
