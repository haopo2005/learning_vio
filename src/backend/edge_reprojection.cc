...
//    std::cout << pts_i_.transpose() <<" "<<pts_j_.transpose()  <<std::endl;
    ...
    Vec3 pts_imu_i = qic * pts_camera_i + tic;//i-->ext, ext其实是I时刻的IMU坐标系
    Vec3 pts_w = Qi * pts_imu_i + Pi;//I时刻IMU-->I时刻世界坐标系
    Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);//I与J时刻世界坐标系3D点残差，在J时刻IMU坐标系下
    Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic);//J时刻IMU坐标系下残差-->J时刻相机坐标系残差
