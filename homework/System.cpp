    ...
    :bStart_backend(true)
{
    ...
    string sConfig_file = sConfig_file_ + "sim.yaml";

    cout << "1 System() sConfig_file: " << sConfig_file << endl;
    readParameters(sConfig_file);

    trackerData[0].readIntrinsicParameter(sConfig_file); //NUM_OF_CAM=1
...
void System::PubImageDataSim(double dStampSec, std::vector<cv::Point2f> &featurePoint)
{
    pub_count++;
    ...
    {
        for(unsigned int j=0;j<featurePoint.size();j++)
        {
            auto &un_pts = featurePoint[j];
        ...
        if(!init_pub)
        {
            cout<<"skip the first image; since no optical speed on first image"<<endl;
            init_pub = 1;
        }else
        {
            m_buf.lock();
...
void System::PubImageData(double dStampSec, Mat &img)
{
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            auto &pts_velocity = trackerData[i].pts_velocity;//来自于光流的像素移动，见undistortedPoints函数
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    double x = un_pts[j].x; //归一化相机平面坐标，光流/特征点
                    double y = un_pts[j].y;
                    double z = 1;
...
vector<pair<vector<ImuConstPtr>, ImgConstPtr>> System::getMeasurements()
{
    ...
    while (true)
    {
        ...
        {
            cout<<imu_buf.back()->header<<","<<feature_buf.front()->header<<","<<estimator.td<<endl;
        ...
        {
            cout<<imu_buf.front()->header<<","<<feature_buf.front()->header<<","<<estimator.td<<endl;
...
void System::ProcessBackEnd()
{
    ...
    {
        // cout << "1 process()" << endl;
        vector<pair<vector<ImuConstPtr>, ImgConstPtr>> measurements;//关于pair的数组
        ...
        lk.unlock();
        m_estimator.lock(); //unlock等for循环结束
        for (auto &measurement : measurements)  //measurement.second: img points, measurement.first: IMU, 角速度及角速度
        {
